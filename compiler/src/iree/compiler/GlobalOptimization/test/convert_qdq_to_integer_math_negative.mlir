...
// N * (|Aq|max + |zA|max) * (|Bq|max + |zB|max). Storage and zero-point widths
// enter the bound separately per operand, and a width at or above the
// accumulator's admits no bound at all. The admissible counterpart of each pair
// below is in convert_qdq_to_integer_math_algebra.mlir.
//===----------------------------------------------------------------------===//
...
// An unconstrained i32 zero point cannot be bounded by the i8 storage range, so
// the operand admits no magnitude even though its data is narrow.
func.func @wide_zero_point(%aq: tensor<4x8xi8>, %a_s: f32, %a_z: i32, %bq: tensor<8x16xi8>, %b_s: f32, %b_z: i8) -> tensor<4x16xf32> {
  %a_i = tensor.empty() : tensor<4x8xf32>
  %a = iree_linalg_ext.dequantize_affine
      {input_unsigned,
       indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
...
// CHECK-LABEL: func.func @wide_zero_point(
//       CHECK:   iree_linalg_ext.dequantize_affine
//       CHECK:   linalg.matmul
